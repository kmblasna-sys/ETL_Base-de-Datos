...
try:
    from validaciones_y_graficas.Laurente import obtener_lotes_perecederos
...
class PanelLaurenteApp(tb.Frame):
    """Panel de vencimientos y salida prioritaria integrable en el contenedor principal."""
    ...
    def crear_layout(self):
        ...
        tb.Label(
            header,
            text="📦 Panel de Vencimientos - Lotes en Riesgo",
        ...
        self.lbl_info = tb.Label(
            info_bar,
            text="Presiona 'Cargar datos' para mostrar la grilla y el gráfico.",
            font=("Helvetica", 10),
        )
        ...
        self.cmb_criterio = ttk.Combobox(
            row,
            state="readonly",
            values=[
                "Todos",
                "Próximos a vencer (<= 180 días)",
                "Críticos (< 60 días)",
                "Vencidos / caducados",
            ],
            width=28,
        ...
        tb.Button(
            derecho,
            text="🚨 Disparar salida prioritaria",
            bootstyle="danger",
            command=self.disparar_salida_prioritaria,
        ).pack(anchor=W, pady=(8, 0))

    def cargar_datos(self):
        if not obtener_lotes_perecederos:
            mensaje = "No se encontró la función de validación en Laurente.py."
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudieron cargar los lotes perecederos.\n\nDetalle: {e}")
            return

        if not self._datos:
            self.lbl_info.config(text="No se encontraron datos de lotes perecederos o no hay conexión.")
            self.lbl_estado.config(text="Estado: sin datos.")
            self.mostrar_datos([])
            return

        self.lbl_info.config(
            text=f"Datos cargados: {len(self._datos)} lotes. Busca por nombre o código y aplica el criterio de caducidad."
        )
        self.lbl_estado.config(text=f"Estado: {len(self._datos)} lotes cargados.")
    ...
    def mostrar_datos(self, datos):
        ...
        for fila in datos:
            ...
            item_id = self.tree.insert(
                "",
                END,
                values=(lote, codigo, producto, fecha_ingreso, fecha_vencimiento, dias, prioridad, capacidad, ocupado),
            )
    ...
    def _aplicar_colores(self):
        self.tree.tag_configure("vencido", background="#f5b7b1")
    ...
    def aplicar_filtro(self):
        ...
        if texto_busqueda:
            self.lbl_info.config(
                text=f"Mostrando {len(datos_filtrados)} lotes que coinciden con '{texto_busqueda}' y criterio '{criterio}'."
            )
        else:
    ...
    def ver_grafico(self):
        ...
        valores = [
            max(0, fila.get("Dias_Restantes", 0))
            for fila in self._datos
            if fila.get("Dias_Restantes") is not None
        ]
        if not valores:
        ...
        canvas.create_text(
            margin_x + width / 2,
            20,
            text="Histograma de Días Restantes para Vencimiento",
        ...
        for linea in [60, 180]:
            if min_val <= linea <= max_val:
                ...
                canvas.create_line(
                    ...
                    x,
                    margin_y + height,
                    fill="#e74c3c" if linea == 60 else "#f39c12",
                    dash=(4, 4),
                )
                canvas.create_text(
                    x + 4,
    ...
    def disparar_salida_prioritaria(self):
        ...
        messagebox.showinfo(
            "Salida prioritaria",
            f"Los lotes {', '.join(map(str, lotes))} fueron marcados para salida prioritaria.",
        )
