...
# CAMBIO: Ahora hereda de tb.Frame para poder ser importado e incrustado
class PanelLaurente(tb.Frame):
    def __init__(self, master, **kwargs):
        # Inicializa como un Frame hijo del contenedor
    ...
    def crear_layout(self):
        # CAMBIO: Todo lo que antes usaba 'self.root' ahora usa 'self'
        ...
        filtros_frame = tb.Labelframe(self, text="Filtros dinámicos por rango de días de vencimiento", padding=12)
        filtros_frame.pack(fill=X, padx=12, pady=(0, 8))

        row = tb.Frame(filtros_frame)
        row.pack(fill=X, pady=4)
        tb.Label(row, text="Días mínimos:", width=14).pack(side=LEFT)
        self.ent_dias_min = tb.Entry(row, width=12)
        self.ent_dias_min.insert(0, "0")
        self.ent_dias_min.pack(side=LEFT, padx=(0, 14))

        tb.Label(row, text="Días máximos:", width=14).pack(side=LEFT)
        self.ent_dias_max = tb.Entry(row, width=12)
        self.ent_dias_max.insert(0, "180")
        self.ent_dias_max.pack(side=LEFT, padx=(0, 14))

        tb.Button(row, text="Filtrar rango", bootstyle="success", command=self.aplicar_filtro).pack(side=LEFT)
        ...
        self.tree = tb.Treeview(panel, columns=("lote", "fecha_ingreso", "dias_restantes", "prioridad", "capacidad", "ocupado"), show="headings", bootstyle="secondary", height=18)
        for col, text, width in [
            ("lote", "Número de Lote", 150),
            ("fecha_ingreso", "Fecha Ingreso", 120),
            ("dias_restantes", "Días Restantes", 120),
            ("prioridad", "Prioridad", 120),
            ("capacidad", "Capacidad Total", 130),
            ("ocupado", "Espacio Ocupado", 130),
        ]:
    ...
    def cargar_datos(self):
        ...
        self.lbl_info.config(text=f"Datos cargados: {len(self._datos)} lotes. Ajusta el rango y presiona 'Filtrar rango'.")
        self._datos_lotes = []
    ...
    def mostrar_datos(self, datos):
        ...
        for fila in datos:
            ...
            item_id = self.tree.insert("", END, values=(lote, fecha_ingreso, dias, prioridad, capacidad, ocupado))
            self._datos_lotes.append((item_id, lote, dias, prioridad))
    ...
    def _aplicar_colores(self):
        ...
        for item_id, lote, dias, prioridad in self._datos_lotes:
            tag = 'critico' if prioridad == 'Crítico' else 'alerta' if prioridad == 'Alerta' else 'normal'
    ...
    def aplicar_filtro(self):
        ...
        try:
            dias_min = int(self.ent_dias_min.get())
            dias_max = int(self.ent_dias_max.get())
        except ValueError:
            messagebox.showerror("Filtro", "Ingresa valores numéricos válidos para el rango de días.")
            return

        if dias_max < dias_min:
            messagebox.showerror("Filtro", "El valor máximo debe ser mayor o igual al mínimo.")
            return

        datos_filtrados = [
            fila for fila in self._datos
            if fila.get('Dias_Restantes', -1) >= dias_min and fila.get('Dias_Restantes', -1) <= dias_max
        ]
        self.lbl_info.config(text=f"Mostrando {len(datos_filtrados)} lotes entre {dias_min} y {dias_max} días restantes.")
    ...
    def ver_grafico(self):
        ...
        # CAMBIO: Usamos self.winfo_toplevel() para encontrar la ventana raíz activa de forma segura
    ...
    def disparar_salida_prioritaria(self):
        ...
        lotes = [self.tree.item(item)['values'][0] for item in seleccion]
...
# Mantener la ejecución independiente por si quieres probar este archivo por separado
if __name__ == '__main__':
    root = tb.Window(themename='litera')
    root.title("PanelLaurente - Vencimientos y Salida Prioritaria")
    root.geometry("1120x720")
    
    app = PanelLaurente(root)
